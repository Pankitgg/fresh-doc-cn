---
description: |
  了解如何使用 Deno 的内置测试运行器测试 Fresh 应用。
---

为确保您的应用按预期工作，我们可以编写测试。Fresh 的任何方面都可以整体测试或单独测试。我们使用 Deno 的内置
[测试运行器](https://docs.deno.com/runtime/fundamentals/testing/) 来编写测试。

## 测试中间件

要测试 [中间件](/docs/concepts/middleware)，我们将创建一个虚拟应用并在自定义的 `/` 处理器中返回我们要检查的相关信息。此测试假设 `utils.ts` 中的 `State` 对象具有 `text` 属性。
...
[应用包装器](/docs/advanced/app-wrapper) 组件和
[布局](/docs/advanced/layouts) 都可以以相同的方式测试。
...
要测试您的路由处理器和业务逻辑，您可以使用上面显示的相同
[`App`](/docs/concepts/app) 模式。Fresh 使测试单个路由变得容易，无需完整的构建过程，只要它们导出一个处理器：
...
您可以使用相同的 [`App`](/docs/concepts/app) 模式测试您的 islands 在服务器上是否正确渲染。注意：这需要 `.tsx` 文件扩展名才能使用 JSX：
