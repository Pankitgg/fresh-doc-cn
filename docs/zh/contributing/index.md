...
- **`packages/fresh/`** - Fresh 核心框架（`@fresh/core`）
...
前置条件：[Deno](https://deno.com/)（最新版本）和 Git。
...
`deno task ok` 运行格式化、linting、类型检查和完整的测试套件。在提交任何拉取请求之前运行它。

该仓库使用 Deno [工作空间](https://docs.deno.com/runtime/fundamentals/workspaces/)，因此 `packages/` 中的所有包都可以使用其发布名称（`@fresh/core`、`@fresh/plugin-vite` 等）自动相互使用。
...
要在单独的项目中使用您的本地 Fresh 检出版本，请将 [`links`](https://docs.deno.com/runtime/fundamentals/configuration/#links) 添加到项目的 `deno.json`：
...
- 提交前运行 `deno task ok` 以尽早发现格式化、lint 和类型错误
- 保持 PR 专注——每个 PR 一个功能或修复
- 为任何行为更改添加或更新测试
- 遵循现有代码风格——仓库使用 `deno fmt` 进行格式化，`deno lint` 进行 linting
