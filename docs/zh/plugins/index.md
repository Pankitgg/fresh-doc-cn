...
如果你需要修改请求、添加 HTTP 头或通过 `ctx.state` 向其他[中间件](/zh/concepts/middleware)传递额外数据，那么使用中间件是最佳选择。
...
了解更多关于[中间件](/zh/concepts/middleware)的信息。
...
对于更复杂的插件，你可以组合多个中间件、添加路由，或使用[`Builder`](/zh/advanced/builder)钩子进行构建时处理。

## 内置插件

Fresh 自带以下插件：

- [cors()](/zh/plugins/cors) - 设置 CORS HTTP 头
- [csrf()](/zh/plugins/csrf) - CSRF 保护
- [csp()](/zh/plugins/csp) - 内容安全策略头
- [trailingSlashes()](/zh/plugins/trailing-slashes) - 强制尾部斜杠行为
